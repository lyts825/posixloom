...
async function readStandardInput(): Promise<string> {
  ...
  for await (const chunk of process.stdin) {
    ...
    bytes += buffer.length;
    if (bytes > 1024 * 1024) throw new Error("Shell stdin script exceeds 1 MiB");
    chunks.push(buffer);
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (args[0] === "trace") {
      if (args[1] !== "list" && args[1] !== "summary") throw new Error("Use: posixloom trace list|summary [--limit n] [--json]");
      const limitIndex = args.indexOf("--limit");
      const limit = limitIndex >= 0 ? Number(args[limitIndex + 1]) : args[1] === "summary" ? 1000 : 50;
      const events = await readTraceEvents(runtime.config.dataRoot, limit);
      if (args[1] === "summary") console.log(JSON.stringify(summarizeTraces(events), null, 2));
      else if (args.includes("--json")) console.log(JSON.stringify({ path: traceFilePath(runtime.config.dataRoot), events }, null, 2));
